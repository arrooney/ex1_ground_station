...
static gomshellRingAdd( struct CIList* tail_list, const char* tail_file_name)
{
	size_t i;
...
static void gomshellFetchRingByID( cell_t id )
{
	...
	FILE* meta_file;
	static char* ring_meta_data_path[] = { "/sd/MT-ALog.txt", /* DFGM FIR stream A */
...
static void gomshellRingFetch( )
{
	...
	for( i = 0; i < GOMSHELL_TOTAL_RINGS; ++i ) {
		 ...
		 */
		struct CIList* name;
		CIList_Get(&ring_tails.cilist, &name, i);
...
size_t volatile true_packet_index = 0;
static void dfgm_s16tobe( int16_t val, uint8_t* ar )
{
	ar[0] = (uint8_t) (((val & 0xFF00) >> 8) & 0x00FF);
	ar[1] = (uint8_t) (val & 0x00FF);
}
static int16_t dfgm_betos16( uint8_t* ar )
{
	int16_t s = (int16_t) ((((uint16_t) ar[0]) << 8)
			       + ((uint16_t) ar[1]));
	return s;
}
static void dfgm_u32tobe( uint32_t val, uint8_t* ar )
{
...
static uint32_t dfgm_betou32( uint8_t* ar )
{
...
static void dfgm_mock_packet_value( )
{
...
static void dfgm_mock_thread_function( void* arg )
{
	...
	for( ;; ) {
		if( dfgm_true_packet != NULL ) {
			if( serial_putstr(dev, dfgm_true_packet + true_packet_index, 1248) != 1248 ) {
		...
		if( serial_putstr(dev, dfgm_mock_packet, 1248) != 1248 ) {
			sdTerminalPrint("dfgm mock: error writing to uart\n");
			sdTerminalFlush();
		}
		++dfgm_mock_packet[2];
...
static void dfgm_mock_min_max_toggle( )
{
	size_t i, j;
...
static void dfgm_mock_use_ring( cell_t start_, cell_t count_, cell_t data_ )
{
	size_t i;
	...
	if( dfgm_true_packet != NULL ) {
		sdTerminalOut("DFGM Mock: previous packets still being transmitted");
		return;
	}
...
static void incrementRingName( cell_t string_, cell_t length_ )
{
...
CFunc0 CustomFunctionTable[] =
{
    ...
    (CFunc0) dfgm_mock_packet_value,
    (CFunc0) incrementRingName,
    (CFunc0) dfgm_mock_use_ring,
    (CFunc0) dfgm_mock_min_max_toggle
};
#endif
