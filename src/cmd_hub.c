...
int cmd_hub_hk(struct command_context *ctx) {
	...
	printf("Reset  %d\t", nanohub_hk.reset);
	switch(nanohub_hk.reset)
	{
	case 0:
...
int cmd_hub_dio(struct command_context *ctx) {

	char * args = command_args(ctx);
	unsigned int cmd, iod, pin, val;
	uint8_t ret;
	if (sscanf(args, "%u %u %u %u", &cmd,&iod,&pin,&val) == 4) {
		printf("%d %d %d %d\r\n",cmd,iod,pin,val);
		hub_dio((uint8_t)cmd, (uint8_t)iod, (uint8_t)pin, (uint8_t)val, &ret);
		printf("ret 0x%X \r\n",ret);
	}
