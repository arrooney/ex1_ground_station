...
void task_ftp(void * conn_param) {
	...
	while(1) {
				...
				for (i = 0; i < status->entries; i++) {
					...
					for (j = 0; j < status->entry[i].count; j++) {
						...
						if (backend->chunk_read(backend_state, ftp_data_packet->data.chunk, ftp_data_packet->data.bytes, size) != FTP_RET_OK) {
							...
							ftp_data_packet->type = FTP_RET_IO;
		...
		case FTP_MKFS_REQUEST: {
			/* Validate state */
			if (state != FTP_STATE_IDLE) {
				printf("MKFS received in state %"PRIu8"\r\n", state);
				goto out_free;
			}
			...
			if (backend && backend->mkfs) {
				ftp_packet->mkfsrep.ret = backend->mkfs(backend_state, ftp_packet->mkfs.dev);
			} else {
				ftp_packet->mkfsrep.ret = FTP_RET_NOTSUP;
			}

			/* Send back reply */
			ftp_packet->type = FTP_MKFS_REPLY;
			packet->length = sizeof(ftp_type_t) + sizeof(ftp_mkfs_reply_t);
		...
		case FTP_ZIP_REQUEST: {
			/* Validate state */
			if (state != FTP_STATE_IDLE) {
				printf("ZIP received in state %"PRIu8"\r\n", state);
				goto out_free;
			}
			...
			if (backend && backend->zip) {
				...
				ftp_packet->ziprep.ret = FTP_RET_NOTSUP;
			}
			...
			packet->length = sizeof(ftp_type_t) + sizeof(ftp_zip_reply_t);
