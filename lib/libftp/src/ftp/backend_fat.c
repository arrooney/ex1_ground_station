...
static int file_crc(uint32_t * crc_arg) {
	...
	for (i = 0; i < file_size / 256; i++) {
		if ((result = f_read(fd_file, bytes, 256, &read)) != FR_OK)
			return -1;
		for (j = 0; j < 256; j++)
			crc = chksum_crc32_step(crc, bytes[j]);
	}
	for (i = 0; i < file_size % 256; i++) {
		if ((result = f_read(fd_file, &byte, 1, &read)) != FR_OK)
			return -1;
...
ftp_return_t ftp_fat_upload(void * state, const char const * path, uint32_t memaddr, uint32_t size, uint32_t chunk_size) {
	...
	/* Allocate memory map */
	if (mem_map == NULL) {
		mem_map = pvPortMalloc(file_chunks);
		if (mem_map == NULL) {
			f_close(fd_file);
			fd_file = NULL;
...
ftp_return_t ftp_fat_read_chunk(void * state, uint32_t chunk, uint8_t * data, uint32_t size) {
  unsigned int read;
	...
	if (f_read(fd_file, data, size, &read) != FR_OK)
		goto chunk_read_error;
...
ftp_return_t ftp_fat_remove(void *state, char *path) {

	if (f_unlink(path) != F_OK) {
		return FTP_RET_NOENT;
	} else {
		return FTP_RET_OK;
	}
...
ftp_return_t ftp_fat_move(void *state, char *from, char *to) {

	if (f_rename(from, to) != F_OK) {
		return FTP_RET_NOENT;
	} else {
		return FTP_RET_OK;
	}
...
ftp_return_t ftp_fat_mkfs(void *state, uint8_t dev) {
	if (f_mkfs(dev, 0, 0) != F_OK) {
		return FTP_RET_NOENT;
	} else {
		return FTP_RET_OK;
	}
